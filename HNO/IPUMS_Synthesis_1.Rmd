...
for(i in 1:m){
  ...
  synthetic_part1 <- synthesize_catIncome(params, 3000 + i, n)
  names(synthetic_part1) <- c("CatIncome", "CatIncomeSyn")
  synthetic_m[[i]] <- synthetic_part1
}
q <- rep(NA, m)
v <- rep(NA, m)
for (i in 1:m){
  synthetic_part1 <- synthetic_m[[i]]
  q[i] <- mean(synthetic_part1$CatIncomeSyn)
  v[i] <- var(synthetic_part1$CatIncomeSyn)/n
}
```
...
```
Synthetic mean estimand: `r q_bar_m`
Synthetic 95% CI: `r c(q_bar_m - t_score_syn * sqrt(T_p), q_bar_m + t_score_syn * sqrt(T_p))`
...
set.seed(123)
mean_org <- mean(synthetic_part1$CatIncome)
sd_org <- sd(synthetic_part1$CatIncome)
t_score_org <- qt(p = 0.975, df = n-1)
mean_org
```

Original mean estimand: `r mean_org`
...
Synthetic 95% CI: `r c(mean_org - t_score_org * sd_org / sqrt(n), mean_org + t_score_org * sd_org / sqrt(n))`
...
```{r}
## Need to find code to use instead of values
L_s = 0.9324523
U_s = 0.9750220
L_o = 0.9325870
U_o = 0.9746295
...
IPUMSdata_org <- cbind(data, synthetic_part1$CatIncome)
## Bind CatIncomeSyn to synthetic data
IPUMSdata_syn <- cbind(data, synthetic_part1$CatIncomeSyn)

## Rename CatIncome and CatIncomeSyn
colnames(IPUMSdata_org)[colnames(IPUMSdata_org) == "synthetic_part1$CatIncome"] <- "income"
colnames(IPUMSdata_syn)[colnames(IPUMSdata_syn) == "synthetic_part1$CatIncomeSyn"] <- "income"
IPUMSdata_org <- as.data.frame(IPUMSdata_org)
IPUMSdata_syn <- list(as.data.frame(IPUMSdata_syn))
```
...
```{r}
 known.vars <- c("SEX$.data_2", "SEX$.data_1", "RACE$.data_2", "RACE$.data_1","RACE$.data_5","RACE$.data_4", "RACE$.data_3","AGE", "EDUC$.data_3", "EDUC$.data_1", "EDUC$.data_2","HOURSWRK", "HEALTH$.data_1","HEALTH$.data_2","HRSLEEP",
"WORRY$.data_3", "WORRY$.data_5", "WORRY$.data_4", "WORRY$.data_1", "WORRY$.data_2")
 syn.vars <- c("income")
 n <- dim(IPUMSdata_org)[1]
 m <- 1
output <- IdentificationRisk(IPUMSdata_org, IPUMSdata_syn, 
...
```{r}
dataNoZeros = as.data.frame(data[!data$EARNIMPOINT1 == 0,])
...
for (i in 1:nrow(synthetic_one)){
  if (synthetic_one$SynIncome[i] == 0) {
...
data_syn <- as.data.frame(data_syn)
```
