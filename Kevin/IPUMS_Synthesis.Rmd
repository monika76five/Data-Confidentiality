...
-figure out why synthesis is sticky in second part
-create synthesis function as the following:

For each non-zero income produced in part A synthesis:
  plug the values (i.e. x_age, x_sex, etc) for the given non-zero income row into synthesis function
  synthesize income value based on params from second model and values from line above
  store value
  
-add evaluation / risk disclosure 
...
v <- rep(NA, m)
for (i in 1:m){
  synthetic_one <- synthetic_one[[i]]
  q[i] <- mean(synthetic_one$SynLogIncome)
  v[i] <- var(synthetic_one$SynLogIncome)/n
}
```
